/* global ID, BLOG_ID, NAME, KEYWORD, QR_CODE, RANDOM, LOCK_TOC, LIB_URL, CSS_URL, EXCLUDES, REVERSE, INTERVAL, EXPIRES, HEIGHT, TYPE, BASEURL, ALLOW_MOBILE */
...
export default {
  ...
  methods: {
    // 加载引流插件
    loadReadmorePlugin(waitMills) {
      // 利用定时器来保证可以正常操作DOM节点
      setTimeout(() => {
        this.updateReadmorePlugin();
      }, waitMills);
    },
      ...
      // 更新引流插件
        ...
        if (divArr && divArr.length > 0) {
          ...
          asyncLoadJs(LIB_URL, 'readmore-js', () => {
            // 初始化引流插件
            initPlugin(ID, BLOG_ID, NAME, KEYWORD, QR_CODE, RANDOM, LOCK_TOC, INTERVAL, EXPIRES, HEIGHT, TYPE, BASEURL, ALLOW_MOBILE);
          });
        } else {
